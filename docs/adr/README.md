...
Les huit décisions bloquantes du backlog (`D-03` à `D-20`) sont couvertes. Deux d'entre
elles ont été tranchées **contre** la recommandation du backlog : l'ADR-0001 (projets
partagés) et l'ADR-0007 (broker externe). Les deux ADR portent la raison qui a emporté la
décision et le coût accepté en échange.
...
prise : l'ADR-0007 a retenu un broker sans statuer sur ce qui garantit qu'un événement
publié corresponde à un fait réellement écrit. Il est daté et rattaché à `EN-35`.
