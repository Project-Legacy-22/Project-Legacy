...
const RULES = [
    ...
    },
    {
        ...
        bare: [/^@legacy\/(contracts|core-items|core-auth|core-projects)$/, /^node:/, /^(@supabase\/supabase-js|pino)$/],
        local: /^\.\.?\//,
    },
    {
        name: 'only the composition root reaches for an adapter',
        match: /^apps\/api\/src\/(?!composition-root\.ts$)/,
        bare: [/^@legacy\/(contracts|core-items|core-auth|core-projects)$/, /^node:/, /^(express|zod|uuid)$/],
        local: /^\.\.?\//,
    },
    {
